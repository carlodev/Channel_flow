"""
Channel multicore
Problem: get_cell_measure not defined for distributed Geometry and using the h_cell function does not work, ok only for serial
it works in multicore without stabilization parameters, the problem is probablly how h is computed
...
function main_ex1(parts)
    ...
    model = mesh_channel(; D=D, N=N, parts=parts, printmodel=false, periodic=periodic)
...
println(get_cell_points(Ω)) 
cellpoints = get_cell_points(Ω)
@save "Cell_points.jld2" cellpoints
...
#mpiexecjl --project=. -n 4 julia Ch_distributed1.jl